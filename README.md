# @shapeshift-labs/frontier-annotations-harness
...
```sh
npm install @shapeshift-labs/frontier-annotations-harness
```

## Browser Harness

```sh
frontier-annotations-harness serve \
  --url http://localhost:5173/editor \
  --source src \
  --verification "npm test" \
  --out agent-runs/browser-annotations \
  --run-swarm \
  --semantic-import
```
...
```sh
frontier-annotations-harness plan \
  --annotation annotation.json \
  --source src \
  --verification "npm test" \
  --out agent-runs/save-button-annotation
```
...
} from '@shapeshift-labs/frontier-annotations-harness';

const harness = await startFrontierAnnotationBrowserHarness({
  url: 'http://localhost:5173/editor',
  sourceRoots: ['src'],
  verification: ['npm test'],
  runSwarm: true
...
`frontier-annotations-harness` is an incubation node package. It depends on `frontier-annotations` for overlay records/context planning, while Playwright and Loom stay optional host tools.
...
The package benchmark measures source scanning and annotation-to-swarm artifact planning against synthetic app sources.
